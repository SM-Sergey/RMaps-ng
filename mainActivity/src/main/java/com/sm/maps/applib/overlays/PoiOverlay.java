...
public class PoiOverlay extends TileViewOverlay {
	...
	public void onDraw(Canvas c, TileView mapView) {
		final com.sm.maps.applib.view.TileView.OpenStreetMapViewProjection pj = mapView.getProjection();
		...
		if (this.mItemList != null) {
			...
			for (int i = this.mItemList.size() - 1; i >= 0; i--) {
					...
					onDrawItem(c, iid, curScreenCoords);

					c.restore();
			}

			if (item_tap != null) {
				pj.toPixels(item_tap.GeoPoint, curScreenCoords);
	...
	protected void onDrawItem(Canvas c, int id, Point screenCoords) {
		...
		if (id == mTapId) {
			...
			pic.setImageResource(Ut.IconResId(focusedItem.IconId));
		...
		if(mBtnMap.indexOfKey(Ut.IconResId(focusedItem.IconId)) > 0)
			marker = mBtnMap.get(Ut.IconResId(focusedItem.IconId));
		else {
			try{
				marker = mCtx.getResources().getDrawable(Ut.IconResId(focusedItem.IconId));
			} catch (Exception e) {
				marker = mCtx.getResources().getDrawable(R.drawable.poi);
			}
			mBtnMap.put(Ut.IconResId(focusedItem.IconId), marker);
		}
