...
public class CoordFormatter {
	...
	public static final String HINT0 = "-00.0000000";
	public static final String HINT1 = "-00 00.000000";
	public static final String HINT2 = "-00 00 00.0000";
	...
	private String convert(double coord, char minus, char plus) {
		...
		if(mFormat == 1) {
			ret = String.format(Locale.UK, "%s%d°%.6f'", coord<0?"-":"", Math.abs((int)coord), Math.abs((coord-(int)coord)*60.0));
		} else if(mFormat == 2) {
			final double min = (coord-(int)coord)*60.0;
			ret = String.format(Locale.UK, "%s%d°%d'%.4f\"", coord<0?"-":"", Math.abs((int)coord), Math.abs((int)min), Math.abs((min-(int)min)*60.0));
		} else if(mFormat == 3) {
			ret = String.format(Locale.UK, "%s%.7f°", coord<0?minus:plus, Math.abs(coord));
		} else if(mFormat == 4) {
			ret = String.format(Locale.UK, "%s%d°%.6f'", coord<0?minus:plus, Math.abs((int)coord), Math.abs((coord-(int)coord)*60.0));
		} else if(mFormat == 5) {
			final double min = (coord-(int)coord)*60.0;
			ret = String.format(Locale.UK, "%s%d°%d'%.4f\"", coord<0?minus:plus, Math.abs((int)coord), Math.abs((int)min), Math.abs((min-(int)min)*60.0));
		} else {
			ret = String.format(Locale.UK, "%.7f°", coord);
		} 
