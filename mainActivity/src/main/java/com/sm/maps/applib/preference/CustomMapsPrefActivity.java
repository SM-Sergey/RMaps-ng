...
import com.sm.maps.applib.utils.OnlineCachePreference;
...
public class CustomMapsPrefActivity extends MMPreferenceActivity implements OnSharedPreferenceChangeListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		{
			final EditTextPreference pref = new EditTextPreference(this);
			pref.setKey(mKey + "_blanktile");
			pref.setTitle(getString(R.string.pref_usermap_blanktile));
			pref.setSummary(bundle.getString(BLANKTILE));
			pref.setDefaultValue(bundle.getString(BLANKTILE));
			pref.setDialogMessage(R.string.pref_custommap_dialogmessage_md5);
			prefscr.addPreference(pref);
		}
		{
		...
		{
			final ListPreference pref = new ListPreference(this);
			pref.setKey(mKey + "_maxdnld");
			pref.setTitle(getString(R.string.pref_maxdnld));
			pref.setEntryValues(R.array.zoomlevel_pref_value);
			pref.setEntries(R.array.zoomlevel_pref_title);
			pref.setValue(""+bundle.getInt(MAXDNLD));
			prefscr.addPreference(pref);
			pref.setSummary(pref.getEntry());
		}
		{
	...
	public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
		if(key.startsWith(PREF_MIXMAPS_)) {
			...
			} else if(key.endsWith(BLANKTILE)) {
				final JSONObject json = MixedMapsPreference.getMapCustomParams(mMapHelper.PARAMS);
				try {
					json.put(BLANKTILE, sharedPreferences.getString(key, ""));
					mMapHelper.PARAMS = json.toString();
					if(findPreference(key) != null)
						findPreference(key).setSummary(sharedPreferences.getString(key, ""));
				} catch (Exception e) {
			...
			} else if(key.endsWith("_maxdnld")) {
				final JSONObject json = MixedMapsPreference.getMapCustomParams(mMapHelper.PARAMS);
				try {
					json.put(MAXDNLD, Integer.parseInt(sharedPreferences.getString(key, "20")));
					mMapHelper.PARAMS = json.toString();
					if(findPreference(key) != null)
						findPreference(key).setSummary(((ListPreference)findPreference(key)).getEntry());
				} catch (Exception e) {
