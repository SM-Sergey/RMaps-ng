...
import com.sm.maps.applib.view.TileView.OpenStreetMapViewProjection;
...
public class MyLocationOverlay extends TileViewOverlay {
	...
	public MyLocationOverlay(final Context ctx){
		...
		mPaintCross.setColor(0xFF7F7F7F);
		PorterDuff.Mode mode = PorterDuff.Mode.XOR;
		mPaintCross.setXfermode(new PorterDuffXfermode(mode));
		...
		int dpi = mCtx.getResources().getDisplayMetrics().densityDpi;
		mCrossSize = dpi/13;
	...
	public void onDraw(final Canvas c, final TileView osmv) {
		if(this.mLocation != null){
			...
			if (mPrefAccuracy != 0
				...
				int PixelRadius = (int) (osmv.getTouchScale() * mAccuracy / ((float)METER_IN_PIXEL / (1 << osmv.getZoomLevel())));
