package com.sm.maps.applib.kml.XMLparser;
...
import com.sm.maps.applib.preference.PredefMapsPrefActivity;
...
public class PredefMapsParser extends DefaultHandler {
	...
	private static final String BLANKTILE = "blanktile";
	...
	private static final String ZOOM_MAXDNLD = "ZOOM_MAXDNLD";
		...
		if(localName.equalsIgnoreCase(MAP)){
			if (mRendererInfo != null) {
				if(attributes.getValue(ID).equalsIgnoreCase(mMapId)){
					...
					mRendererInfo.ZOOM_MAXDNLD = mRendererInfo.ZOOM_MAXLEVEL;
					if(attributes.getIndex(ZOOM_MAXDNLD)>-1)
						mRendererInfo.ZOOM_MAXDNLD = Integer.parseInt(attributes.getValue(ZOOM_MAXDNLD));

					mRendererInfo.BLANKTILE = null;
					if(attributes.getIndex(BLANKTILE)>-1)
						mRendererInfo.BLANKTILE = attributes.getValue(BLANKTILE);
